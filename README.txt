...
Below is now a description of how to use the plotter once it is running. (first buttons, mouse, then top menu bar)



BUTTONS:

One the left is a number of controls. The first drop-down menu is all available data sets. (The example only has one, named 19A-1).
...
The next drop down menu shows various properties of the dataset. The text box shows the value of that property, and the "get", "set" buttons apply to this setting to this dataset. Some common properties:
"marker size"  :  exactly what it says
"color mode"  : how the dataset is colored. If starts with a *, then following text is sent to matplotlib. (e.g., "*b" for a color mode will color all sources blue)
...
"zoom : in" : changes if zooming by-mouse (explained below), zooms in or out.

"1:1 aspect ratio" : when checked, does it's best to keep the aspect ratio of the northing/easting projection constant.
